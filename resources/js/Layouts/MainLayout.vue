    ...
    <div id="MainLayout" class="w-full h-screen">
        <div v-if="$page.url === '/'" id="TopNaveHome" class="fixed z-30 md:hidden block w-full bg-white h-[61px] border-b border-b-gray-300">
            <div class="flex items-center justify-between h-full">
                <Link href="/">
                    <img class="w-[105px] ml-6 cursor-pointer" src="/images/insta-logo.png" alt="instagram logo">
                </Link>
        ...
        <div id="SideNav" class="fixed h-full bg-white xl:w-[280px] w-[80px] md:block hidden border-r border-r-gray-300">
            <Link href="/">
                <img class="xl:hidden block w-[25px] mt-10 ml-[25px] mt-10 ml-[25px] mb-10 cursor-pointer" src="/images/insta-logo-small.png" alt="instagram logo">
                <img class="xl:block hidden w-[120px] mt-10 ml-6 mb-10 cursor-pointer" src="/images/insta-logo.png" alt="instagram logo">
            </Link>
        ...
        <div class="flex lg:justify-between bg-white h-full w-[100%-280px] xl:pl-[280px] xl:pl-[280px] lg:pl-[100px] overflow-auto">
            <div
            ...
            <div v-if="$page.url === '/'" id="SuggestionSection" class="lg:w-4/12 lg:block hidden text-black mt-10">
                <Link :href="route('users.show', { id: $page.props.auth.user.id })" class="flex items-center justify-between max-w-[300px]">
                    <div class="flex items-center">
                        <img class="rounded-full z-10 w-[58px] h-[58px]" :src="$page.props.auth.user.file" :alt="$page.props.auth.user.name + '\'s Profile Picture'">
                        <div class="pl-4">
                ...
                <div v-for="randUser in $page.props.randomUsers" :key="randUser">
                    <Link :href="route('users.show', { id: randUser.id })" class="flex items-center justify-between max-w-[300px] pb-2">
                        <div class="flex items-center">
                            <img class="rounded-full z-10 w-[37px] h-[37px]" :src="randUser.file" :alt="randUser.name + '\'s Profile Picture'">
                            <div class="pl-4">
        ...
        <div id="BottomNav" class="fixed z-30 bottom-0 w-full md:hidden flex items-center justify-around bg-white border-t py-2 border-t-gray-300">
            ...
            <Link :href="route('users.show', { id: $page.props.auth.user.id })">
                <img
                    ...
                    :alt="$page.props.auth.user.name + '\'s Profile Picture'"
                >
            </Link>
        </div>
    </div>

    <CreatePostModal v-if="showCreatePost" @close="$event => showCreatePost = false" />
</template>
